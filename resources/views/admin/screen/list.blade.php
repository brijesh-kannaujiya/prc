          ...
          @endif
              ...
              @if (!empty($buttonFilter))
                  <div class="menu-left">
                      ...
                      <div class="btn-group" id="category_filter_wrap">
                          <select class="form-control" id="category_filter">
                              
                          </select>
                      </div>
                      <div class="btn-group">
                          <a class="btn btn-flat btn-primary" title="{{ trans('admin.sort') }}" id="button_filter">
...
@push('scripts')
  ...
  <script type="text/javascript">
    ...
    $('#button_filter').click(function(event) {
        var url = '{{ $urlFilter??'' }}?select_filter='+$('#order_filter option:selected').val()+"&cat_id="+$('#category_filter option:selected').val();
        $.pjax({url: url, container: '#pjax-container'})
    });
    @endif
...
// get categories on brand id change
function  getCategoryByBrand(){
  let brandID = $('#order_filter').val();
  if(typeof brandID=="undefined" || !brandID){
    return ;
  }
  $.ajax({
                method: 'post',
                url: '<?php echo route('admin.brand.categories')?>',
                data: {
                  'brand_id' : $('#order_filter').val(),
                    _token: '{{ csrf_token() }}',
                },
                success: function (data) {
                  console.log("darta",data);
                  $('#category_filter').html(data.category_opt);
                }
              });
}

$(document).ready(function(){
  console.log('getCategoryByBrand');
  getCategoryByBrand();
});

            ...
            <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main"
                ...
                <div class="box-body">
                    <div class="fields-group">
                        ...
                        <div class="form-group  {{ $errors->has('product_sort_by') ? ' has-error' : '' }}">
                            <label for="product_sort_by"
                                class="col-sm-2  col-form-label">{{ trans('category.admin.product_sort_by') }}</label>
                            <div class="col-sm-8">

                                <select class="form-control product_sort_by select2" style="width: 100%;" name="product_sort_by">
                                    @php
                                        $psb = '';
                                        $psb = !empty($category)?$category['product_sort_by']:'';
                                    @endphp
                                    @foreach ($productSortBy as $key => $value)
                                        <option value="{{ $key }}" {{ $psb === $key ? 'selected':'' }}
                                                >{{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('product_sort_by'))
                                <span class="help-block">
                                    <i class="fa fa-info-circle"></i> {{ $errors->first('product_sort_by') }}
                                </span>
                                @endif
                            </div>
                        </div>

                        <div class="form-group  {{ $errors->has('product_sort_type') ? ' has-error' : '' }}">
                            <label for="product_sort_by"
                                   class="col-sm-2  col-form-label">{{ trans('category.admin.product_sort_type') }}</label>
                            <div class="col-sm-8">

                                <select class="form-control product_sort_by select2" style="width: 100%;" name="product_sort_type">
                                    @php
                                        $pst = '';
                                        $pst = !empty($category)?$category['product_sort_type']:'';

                                    @endphp

                                    @foreach ($productSortType as $key => $value)
                                        <option value="{{ $key }}" {{ $pst === $key ? 'selected':'' }}>{{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                                @if ($errors->has('product_sort_type'))
                                    <span class="help-block">
                                    <i class="fa fa-info-circle"></i> {{ $errors->first('product_sort_type') }}
                                </span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="form-group  kind kind0 kind1  {{ $errors->has('brand_id') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-8">
                                <select class="form-control input-sm brand_id select2" style="width: 100%;"
                                    ...
                                    @php
                                    $cat = 0;
                                    $cat = !empty($category)?$category['brand_id']:0;
                                    @endphp

                                    @foreach ($brands as $k => $v)
                                        <option value="{{ $k }}" {{ $cat==$k ? 'selected':'' }}>{{ $v->name }}
                                        </option>
                                    @endforeach
                                </select>

							...
							<div class="col-sm-8">
								<div class="input-group">
									...
									<input type="text" id="{{ $code }}__name" name="descriptions[{{ $code }}][name]"
										   ...
										   class="form-control input-sm {{ $code.'__name' }}" placeholder="" readonly="readonly"/>
								</div>
						...
						<div class="col-sm-8">
							<select class="form-control input-sm category select2" multiple="multiple"
								...
								@foreach ($repairs as $repair)
									<option value="{{ $repair->id }}" selected>{{ $repair->title}}
									</option>
								@endforeach
							</select>
...
@push('scripts')
	...
	<script type="text/javascript">
		...
		$('#nama').on('change', function(){
			...
			$('#alias').val(prodalias);
			$('input[id$="__keyword"]').val(prodName);
			$('textarea[id$="__description"]').text(prodName);
			$('textarea[id$="__content"]').html(prodName);
			$.ajax({

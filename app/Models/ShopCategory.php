...
class ShopCategory extends Model
{
    ...
    public function getBrand($key){

        return ShopBrand::getOne($key,1);
    }


    public function description()
    {
        return $this->belongsTo(ShopCategoryDescription::class, 'id', 'category_id');
    }
    ...
    */
    ...
    public static function getArrayCategoryName()
    {
        $categories = ShopCategoryDescription::select('category_id', 'title')->get();

        $arrCategory = [];
        foreach ($categories as $key => $category) {
            $arrCategory[$category->category_id] = $category->title . ' (' . $category->title . ')';
        }
        return $arrCategory;
    }
}

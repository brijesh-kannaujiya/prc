...
{
    const BUFFER = 50;  // 10 characters: to show 10 neighbouring characters around the searched word
...
//        $toExclude = [Comment::class];
        ...
        $results = collect($files)->map(function (SplFileInfo $file){
            ...
            if($classname === null || $classname === 'simple_html_dom'){
                return false;
            }
            ...
            $fields = array_filter($model::SEARCHABLE_FIELDS,
                function($field) {
                if($field !== 'id' && $field !== 'product_id' && $field !== 'alias' && $field !== 'url')
                    return $field;
                });
            return $model::search($keyword)->get()->map(function ($modelRecord) use ($model, $fields, $keyword, $classname){
                ...
                $modelRecord->setAttribute('match', $sliced ?? sc_html_render(substr($serializedValues, 0, 50)) . '...');
    ...
    private function resolveModelViewLink(Model $model)
    {
        ...
        $mapping = [
            \App\Models\ShopBrand::class => '/prc/brand/{alias}',
            \App\Models\ShopProduct::class => '/prc/product/{alias}',
            \App\Models\ShopProductDescription::class => '/prc/product/{alias}',
            \App\Models\ShopLink::class => '{url}',
            \App\Models\ShopRepair::class => '/showRepair/{alias}',
            \App\Models\ShopFaq::class => '/faq'
        ...
        if(Arr::has($mapping, $modelClass)){
            if($modelClass == 'App\Models\ShopProductDescription'){
                return URL::to(str_replace('{alias}', $model->product->alias,$mapping[$modelClass]));

            }
            if($model->alias){
                return URL::to(str_replace('{alias}', $model->alias,$mapping[$modelClass]));

            }
            if($model->url){
                return URL::to(str_replace('{url}',sc_url_render($model->url),$mapping[$modelClass]));

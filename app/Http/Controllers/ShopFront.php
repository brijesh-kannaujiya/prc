...
class ShopFront extends GeneralController
{
    ...
    { 
        if ($this->templatePath == 'templates.prc') {
            // Manoj by pass view for product listing page 
            return Bz::showbrand();
            // End Manoj by pass view 
            return view($this->templatePath . '.home');
        } else {
            return view(
    ...
    public function pageDetail($alias)
    {
        ...
        if ($page) {
            return view(
                $this->templatePath . '.mainpage.shop_page',
                array(
                    'title' => $page->title,
                    //'description' => $page->description,
                    'description' => $page->descriptions->first(),
    ...
    public function news()
    {
        ...
        return view(
            $this->templatePath . '.mainpage.shop_news',
            array(
    ...
    public function newsDetail($alias)
    {
        ...
        if ($news) {
            return view(
                $this->templatePath . '.mainpage.shop_news_details',
                array(
                    'title' => $news->title,
                    'news' => $news,
                    'description' => $news->descriptions->first(),
                    'keyword' => $news->keyword,
                    'og_image' => asset($news->getImage()),
                    'layout_page' => 'news_detail',
                    'created_on' => $news->created_at,
                    'newslist' => $newslist,
                )
            );
        } else {
